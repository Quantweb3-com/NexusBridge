...
class TestGeneralClient(IsolatedAsyncioTestCase):
    ...
    async def test_post_dapi_v1_listen_key(self, mock_get):
        # 执行异步 API 请求
        result = await self.client.post_dapi_v1_listen_key()
        print(result)

    @patch('aiohttp.ClientSession.put')
    async def test_put_dapi_v1_listen_key(self, mock_put):
        # 执行异步 API 请求
        result = await self.client.put_dapi_v1_listen_key()
        print(result)

    @patch('aiohttp.ClientSession.delete')
    async def test_delete_dapi_v1_listen_key(self, mock_delete):
        # 执行异步 API 请求
        result = await self.client.delete_dapi_v1_listen_key()
        print(result)
